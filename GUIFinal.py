...
def infoChoice():
    ...
    var = StringVar(g)
    var.set("News")
    choices = OptionMenu(p, var, "News", "Ratio Analysis", "Compare Prices", "Watchlist", "S&P 500", "Investors Guide")
    choices.pack()
...
#Display S&P500 
def outputSP(): 
    spdata = pd.read_html('https://en.wikipedia.org/wiki/List_of_S%26P_500_companies')[0]
    temp = Frame(f)
    temp.pack()
    tbl = Table(temp, dataframe=spdata, editable=False)
    tbl.showIndex() 
    tbl.show()

#Display Investors Guide 
def iGuide():
    temp = Frame(f)
    temp.pack()
    file = open('2053final\investor-guide.txt', 'r')
    for line in file.readlines():
        Label(temp, text=line[:-1]).pack()
